...
export default defineConfig({
  // GitHub Pages hosts the repo at /salesagent-ai/, so the static Pages build
  // must prefix asset URLs with that base. The preview build (no GH_PAGES env)
  // keeps root-relative assets. Selected at build time so neither build affects
  // the other.
  base: process.env.GH_PAGES === "1" ? "/salesagent-ai/" : "/",
  server: {
    port: 3000,
    host: true,
